...
class _StatisticScreenState extends State<StatisticScreen> {
  ...
  double milliSecsToHour(int clientTime) {
    ...
    return time.hour
        .toDouble() + time.minute / 60.toDouble();
  }
  ...
  void initState() {
    ...
    Pref.getInstance().then((pref) {
      ...
      store.getStatisticInfo(GetStatisticParam(user.companyId,formatDateString(_fromDate),formatDateString(_toDate),user.username));
    });
        ...
        for (var detail in store.result.detailList) {
                ...
                barRodsStack.add(
                  BarChartRodStackItem(
                      ...
                      (i + 1 < detail.listCheckInTime.length
                          ? detail.listCheckInTime[i+1].reason.isNotEmpty ? detail
                          .listCheckInTime[i+1].shiftEndTime: milliSecsToHour(
                          detail.listCheckInTime[i + 1].clientTime)
                          : detail
                          .listCheckInTime[i].shiftEndTime),
                      Colors.lightBlue),
                );
              }
              else{
                if(i<detail.listCheckInTime.length-1)
                  ...
                  BarChartRodStackItem(
                      ...
                      (i + 1 < detail.listCheckInTime.length
                          ...
                          detail.listCheckInTime[i + 1].clientTime)
                          :detail
                          .listCheckInTime[i].shiftStartTime),
                      Colors.white),
                );
              }
            } catch(e){
          ...
          if (barRodsStack.isNotEmpty)
            data.add(BarChartGroupData(x: x, barRods: [
              BarChartRodData(
                  y: isComplete
                      ? milliSecsToHour(detail
                          .listCheckInTime[detail.listCheckInTime.length - 1]
                          .clientTime)
                      : detail
                          .listCheckInTime[detail.listCheckInTime.length - 1]
                          .shiftEndTime,
                      ...
                      children: <Widget>[
                                      ...
                                      children: <Widget>[
                                        ...
                                        Align(
                                          ...
                                          child: CircularPercentIndicator(
                                            ...
                                            lineWidth: 10.0,
                                            animation: true,
                                            percent: store.result==null ? 0 : store.result.onTimePercentage.toDouble()/100,
                                            center: Container(
                                              margin: EdgeInsets.only(left: 9),
                                              child: Row(
                                                mainAxisAlignment:
                                                ...
                                                children: <Widget>[
                                                  Text(
                                                    store.result==null ? '0.0' : store.result.onTimePercentage.toStringAsFixed(0),
                                                    style: TextStyle(
                                                        fontSize: 30,
                                                        fontWeight:
                                                        FontWeight.w600),
                            ...
                            InkWell(
                              onTap: () {
                                Navigator.push(context, MaterialPageRoute(
                                    builder: (context) =>
                                        DetailStatisticScreen(date: date, listDetail: store.result.detailList,)),);
                              },
                              child: Container(
                                padding: EdgeInsets.all(5),
                                width: MediaQuery.of(context).size.width/2.15,
                        ...
                        Container(
                          ...
                          child: SingleChildScrollView(
                            ...
                            child: Container(
                              padding: EdgeInsets.only(
                                  left: 12, top: 24, right: 12, bottom: 8),
